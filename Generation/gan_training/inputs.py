...
class SubsetImageFolder(torchvision.datasets.ImageFolder):
    """
    Custom subset of ImageFolder dataset to load only the first n folders.
    """
    def __init__(self,
                 root,
                 transform=None,
                 ...
                 n_folders=10):
        super(SubsetImageFolder, self).__init__(root,loader=loader,
                                                transform=transform,
                                                target_transform=target_transform)
        self.samples = self.samples[:n_folders]
...
def get_dataset(name,
    ...
    if name == 'tiny_imagenet':
        ...
        indice = [i for i in range(len(dataset)) if dataset.targets[i] in range(10)]
        dataset = Subset(dataset, indice)
        nlabels = 10
    elif name == 'webp':
        ...
        dataset=Data_set('dataset_defend_1.h5', transform=transform)
        nlabels = 10
