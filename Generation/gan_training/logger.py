...
class Logger(object):
    ...
    def add(self, category, k, v, epoch_idx):
        ...
        self.stats[category][k].append((epoch_idx, v))

        k_name = '%s/%s' % (category, k)
        if self.monitoring == 'telemetry':
            self.tm.metric_push_async({'metric': k_name, 'value': v, 'epoch_idx': epoch_idx})
        elif self.monitoring == 'tensorboard':
            self.tb.add_scalar(k_name, v, epoch_idx)

    def add_imgs(self, imgs, class_name, epoch_idx):
        outdir = os.path.join(self.img_dir, class_name)
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        outfile = os.path.join(outdir, '%08d.png' % epoch_idx)
        ...
        if self.monitoring == 'tensorboard':
            self.tb.add_image(class_name, copy.deepcopy(imgs), epoch_idx)
