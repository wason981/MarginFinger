...
def build_source_model(dataset):
    source_model = models.vgg16_bn(pretrained=False)
    in_feature = source_model.classifier[-1].in_features
    if dataset=='cifar10':
        source_model.classifier[-1] = torch.nn.Linear(in_feature, 10)
        source_model.load_state_dict(torch.load('../model/cifar10/Source_Model/source_model.pth'))

    if dataset=='tiny_imagenet':
        source_model.classifier[-1] = torch.nn.Linear(in_feature, 100)
        source_model.load_state_dict(torch.load('../model/tiny_imagenet/Source_Model/source_model.pth'))

    source_model = source_model.cuda().eval()
    return source_model
...
class Trainer(object):
    def __init__(self,
        ...
        self.source_model=build_source_model(self.dataset)
    ...
    def generator_trainstep(self, y, z):
        ...
        #########origin loss ##############
        origin_loss =self.compute_loss(d_fake, 1)

        #########margin loss ##############
        y_pred = torch.softmax(self.source_model(transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))(denorm(x_fake))), dim=1)
        ...
        margin_loss = torch.abs(second_pred - right_pred + self.c).mean()
        gloss = origin_loss +self.omega * margin_loss
